...
static int ft847old_send_priv_cmd(RIG *rig, int cmd_index);
...
#define FT847OLD_ALL_RX_MODES (RIG_MODE_AM|RIG_MODE_CW|RIG_MODE_CWR|RIG_MODE_SSB|RIG_MODE_FM)
#define FT847OLD_SSB_CW_RX_MODES (RIG_MODE_CW|RIG_MODE_CWR|RIG_MODE_SSB)
#define FT847OLD_AM_FM_RX_MODES (RIG_MODE_AM|RIG_MODE_FM)
...
#define FT847OLD_OTHER_TX_MODES (RIG_MODE_AM|RIG_MODE_CW|RIG_MODE_SSB|RIG_MODE_FM)
#define FT847OLD_AM_TX_MODES (RIG_MODE_AM)

#define FT847OLD_FUNC_ALL (RIG_FUNC_TONE|RIG_FUNC_TSQL)

#define FT847OLD_LEVEL_ALL (RIG_LEVEL_RAWSTR|RIG_LEVEL_STRENGTH|RIG_LEVEL_ALC)

#define FT847OLD_VFOS (RIG_VFO_MAIN|RIG_VFO_SUB)

/* FT-847 has different antennas connectors, but no rig_set_ant() ability */
#define FT847OLD_ANTS RIG_ANT_1

#define FT847OLD_STR_CAL { 3, \
	{ \
		{   0, -60 }, /* S0 */ \
		{  16,   0 }, /* S9 */ \
		{  31,  60 }, /* +60 dB */ \
 ...
 * c.f. ft847old_set_ctcss_tone()
 */
static const tone_t ft847old_ctcss_list[] = {
...
const struct rig_caps ft847old_caps = {
.rig_model =  RIG_MODEL_FT847OLD,
.model_name = "FT-847 (Early model)",
.mfg_name =  "Yaesu",
.version =  "0.5",
.copyright =  "LGPL",
...
.write_delay =  FT847OLD_WRITE_DELAY,
.post_write_delay =  FT847OLD_POST_WRITE_DELAY,
.timeout =  1000,
.retry =  0,

.has_get_func =  RIG_FUNC_NONE,
.has_set_func =  FT847OLD_FUNC_ALL,
.has_get_level =  FT847OLD_LEVEL_ALL,
.has_set_level =  RIG_LEVEL_NONE,
.has_get_parm =  RIG_PARM_NONE,
.has_set_parm =  RIG_PARM_NONE,
.level_gran =  {}, 		/* granularity */
.parm_gran =  {},
.ctcss_list =  ft847old_ctcss_list,
...
.bank_qty =   0,
.chan_desc_sz =  0,
.str_cal = FT847OLD_STR_CAL,

.chan_list =  { RIG_CHAN_END, }, /* FIXME: memory chan list: 78, but only in clonable mode? */

.rx_range_list1 =  {
    {kHz(100),MHz(30),FT847OLD_ALL_RX_MODES,-1,-1}, /* rx range begin */
    {MHz(36),MHz(76),FT847OLD_ALL_RX_MODES,-1,-1},
    {MHz(108),MHz(174),FT847OLD_ALL_RX_MODES,-1,-1},
    {MHz(420),MHz(512),FT847OLD_ALL_RX_MODES,-1,-1},

    RIG_FRNG_END, }, /* rx range end */

.tx_range_list1 =  {
    FRQ_RNG_HF(1,FT847OLD_OTHER_TX_MODES, W(5),W(100),FT847OLD_VFOS,FT847OLD_ANTS),
    FRQ_RNG_HF(1,FT847OLD_AM_TX_MODES, W(1),W(25),FT847OLD_VFOS,FT847OLD_ANTS),

    FRQ_RNG_6m(2,FT847OLD_OTHER_TX_MODES, W(5),W(100),FT847OLD_VFOS,FT847OLD_ANTS),
    FRQ_RNG_6m(2,FT847OLD_AM_TX_MODES, W(1),W(25),FT847OLD_VFOS,FT847OLD_ANTS),

    FRQ_RNG_2m(2,FT847OLD_OTHER_TX_MODES, W(1),W(50),FT847OLD_VFOS,FT847OLD_ANTS),
    FRQ_RNG_2m(2,FT847OLD_AM_TX_MODES, W(1),W(12.5),FT847OLD_VFOS,FT847OLD_ANTS),

    FRQ_RNG_70cm(2,FT847OLD_OTHER_TX_MODES, W(1),W(50),FT847OLD_VFOS,FT847OLD_ANTS),
    FRQ_RNG_70cm(2,FT847OLD_AM_TX_MODES, W(1),W(12.5),FT847OLD_VFOS,FT847OLD_ANTS),
...
.tx_range_list2 =
  {
    FRQ_RNG_HF(2,FT847OLD_OTHER_TX_MODES, W(5),W(100),FT847OLD_VFOS,FT847OLD_ANTS),
    FRQ_RNG_HF(2,FT847OLD_AM_TX_MODES, W(1),W(25),FT847OLD_VFOS,FT847OLD_ANTS),
...
.tuning_steps =  { {FT847OLD_SSB_CW_RX_MODES,1}, /* normal */
    {FT847OLD_SSB_CW_RX_MODES,10}, /* fast */
    {FT847OLD_SSB_CW_RX_MODES,100}, /* faster */


    {FT847OLD_AM_FM_RX_MODES,10}, /* normal */
    {FT847OLD_AM_FM_RX_MODES,100}, /* fast  */
...
.priv =   NULL,
.rig_init =   ft847old_init,
.rig_cleanup =  ft847old_cleanup,
.rig_open =   ft847old_open,
.rig_close =  ft847old_close,

.set_freq =   ft847old_set_freq,		/* set freq */
.get_freq =  NULL,		/* get freq */
.set_mode =  ft847old_set_mode,		/* set mode */
.get_mode =  NULL,		/* get mode */
.set_split_vfo  = ft847old_set_split_vfo,
.get_split_vfo  = ft847old_get_split_vfo,
.set_split_freq = ft847old_set_split_freq,
.get_split_freq = NULL,
.set_split_mode = ft847old_set_split_mode,
.get_split_mode = NULL,
.set_ptt =  ft847old_set_ptt,		/* set ptt */
.get_ptt =  ft847old_get_ptt,		/* get ptt */
.get_dcd =  ft847old_get_dcd,		/* get dcd */
.get_level = ft847old_get_level,           /* get level */

.set_func       = ft847old_set_func,
.set_ctcss_tone = ft847old_set_ctcss_tone,
.set_ctcss_sql  = ft847old_set_ctcss_sql,
.set_dcs_sql    = ft847old_set_dcs_sql,
.set_rptr_shift = ft847old_set_rptr_shift,
.set_rptr_offs  = ft847old_set_rptr_offs,
...
int ft847old_init(RIG *rig) {
  struct ft847old_priv_data *p;
  ...
  p = (struct ft847old_priv_data *) calloc(1, sizeof(struct ft847old_priv_data));
  if (!p) {
...
/*
 * ft847old_cleanup routine
 * the serial port is closed by the frontend
 */

int ft847old_cleanup(RIG *rig) {
  if (!rig)
    return -RIG_EINVAL;
  ...
  rig_debug(RIG_DEBUG_VERBOSE,"ft847old:ft847old_cleanup called \n");
...
/*
 * ft847old_open  routine
 *
 */

int ft847old_open(RIG *rig) {

  /* Good time to set CAT ON */

  rig_debug(RIG_DEBUG_VERBOSE,"ft847old:ft847old_open called \n");

  return ft847old_send_priv_cmd(rig,FT_847_NATIVE_CAT_ON);
}

/*
 * ft847old_close  routine
 *
 */

int ft847old_close(RIG *rig) {

  /* Good time to set CAT OFF */

  rig_debug(RIG_DEBUG_VERBOSE,"ft847old:ft847old_close called \n");

  /* don't care about return value */
  ft847old_send_priv_cmd(rig,FT_847_NATIVE_CAT_OFF);
...
static int ft847old_send_priv_cmd(RIG *rig, int cmd_index) {
  ...
  if (! ncmd[cmd_index].ncomp) {
    rig_debug(RIG_DEBUG_VERBOSE,"ft847old: Attempt to send incomplete sequence \n");
    return -RIG_EINVAL;
  }
...
{
  struct ft847old_priv_data *p = (struct ft847old_priv_data*)rig->state.priv;
...
int ft847old_set_freq(RIG *rig, vfo_t vfo, freq_t freq) {
  struct rig_state *rig_s;
  ...
  rig_debug(RIG_DEBUG_VERBOSE,"ft847old: requested freq = %"PRIfreq" Hz, vfo=%s\n",
		  freq, rig_strvfo(vfo));
  ...
  rig_debug(RIG_DEBUG_VERBOSE,"ft847old: requested freq after conversion = %"PRIll" Hz \n", from_bcd_be(p_cmd,8)* 10 );
...
int ft847old_set_mode(RIG *rig, vfo_t vfo, rmode_t mode, pbwidth_t width) {
  ...
  rig_debug(RIG_DEBUG_VERBOSE,"ft847old: generic mode = %x \n", mode);
 ...
 */
int ft847old_set_split_vfo(RIG *rig, vfo_t vfo, split_t split, vfo_t tx_vfo)
{
  struct ft847old_priv_data *priv = (struct ft847old_priv_data*)rig->state.priv;
  unsigned char cmd_index;	/* index of sequence to send */
  int ret;
  ...
  ret = ft847old_send_priv_cmd(rig,cmd_index);
  if (ret == RIG_OK)
	  priv->sat_mode = split;
...
int ft847old_get_split_vfo(RIG *rig, vfo_t vfo, split_t *split, vfo_t *tx_vfo)
{
  struct ft847old_priv_data *priv = (struct ft847old_priv_data*)rig->state.priv;
...
int ft847old_set_split_freq(RIG *rig, vfo_t vfo, freq_t freq)
{
	return ft847old_set_freq(rig, RIG_VFO_TX, freq);
}

int ft847old_set_split_mode(RIG *rig, vfo_t vfo, rmode_t mode, pbwidth_t width)
{
	return ft847old_set_mode(rig, RIG_VFO_TX, mode, width);
}
...
int ft847old_set_ptt(RIG *rig, vfo_t vfo, ptt_t ptt) {
  unsigned char cmd_index;	/* index of sequence to send */

  rig_debug(RIG_DEBUG_VERBOSE,"ft847old:ft847old_set_ptt called \n");
  ...
  return ft847old_send_priv_cmd(rig,cmd_index);

}

static int ft847old_get_status(RIG *rig, int status_ci)
{
  struct ft847old_priv_data *p = (struct ft847old_priv_data *) rig->state.priv;
  unsigned char *data;
  int len;
  ...
  case FT_847_NATIVE_CAT_GET_TX_STATUS:
    ...
    break;
  default:
    rig_debug(RIG_DEBUG_ERR, "ft847old_get_status: Internal error!\n");
    return -RIG_EINTERNAL;
  }

  serial_flush(&rig->state.rigport);

  n = write_block(&rig->state.rigport, (char *) ncmd[status_ci].nseq, YAESU_CMD_LENGTH);
  if (n < 0)
    return n;
...
int ft847old_get_ptt(RIG *rig, vfo_t vfo, ptt_t *ptt) {

  struct ft847old_priv_data *p = (struct ft847old_priv_data *) rig->state.priv;
  int n;


  n = ft847old_get_status(rig, FT_847_NATIVE_CAT_GET_TX_STATUS);
  if (n < 0)
    return n;
...
int ft847old_get_dcd(RIG *rig, vfo_t vfo, dcd_t *dcd) {

  struct ft847old_priv_data *p = (struct ft847old_priv_data *) rig->state.priv;
  int n;


  n = ft847old_get_status(rig, FT_847_NATIVE_CAT_GET_RX_STATUS);
  if (n < 0)
    return n;
 ...
 * the FT-847 display
 */
static int ft847old_get_rawstr_level(RIG *rig, value_t *val)
{
  struct ft847old_priv_data *p = (struct ft847old_priv_data *) rig->state.priv;
  int n;
...
*/
static int ft847old_get_smeter_level(RIG *rig, value_t *val)
{
  int n;

  n = ft847old_get_rawstr_level(rig, val);
  if (n < 0)
    return n;
 ...
 */
static int ft847old_get_alc_level(RIG *rig, value_t *val)
{
  struct ft847old_priv_data *p = (struct ft847old_priv_data *) rig->state.priv;
  int n;
 ...
 */
int ft847old_get_level(RIG *rig, vfo_t vfo, setting_t level, value_t * val)
{
  if (vfo != RIG_VFO_CURR)
    return -RIG_ENTARGET;
  ...
  case RIG_LEVEL_STRENGTH:
    return ft847old_get_smeter_level(rig, val);
  case RIG_LEVEL_RAWSTR:
    return ft847old_get_rawstr_level(rig, val);
  case RIG_LEVEL_ALC:
    return ft847old_get_alc_level(rig, val);
  default:
    return -RIG_EINVAL;
  }
...
int ft847old_set_func(RIG *rig, vfo_t vfo, setting_t func, int status)
{
  unsigned char p_cmd[YAESU_CMD_LENGTH]; /* sequence to send */
  int ret;
  ft847old_native_cmd_t fcmd;
...
int ft847old_set_ctcss_tone (RIG *rig, vfo_t vfo, tone_t tone)
{
  unsigned char p_cmd[YAESU_CMD_LENGTH]; /* sequence to send */
  int i, ret;

  /*
   * 39 CTCSS CAT codes corresponding to ft847old_ctcss_list
   */
  static const unsigned char ft847old_ctcss_cat[] = {
...
#define FT847OLD_CTCSS_NB 39

  for (i = 0; i<FT847OLD_CTCSS_NB; i++)
  {
     if (ft847old_ctcss_list[i] == tone)
        break;
  }
  if (i == FT847OLD_CTCSS_NB)
     return -RIG_EINVAL;

  /* get associated CAT code */
  p_cmd[0] = ft847old_ctcss_cat[i];

  return write_block(&rig->state.rigport, (char*)p_cmd, YAESU_CMD_LENGTH);
}

int ft847old_set_ctcss_sql (RIG *rig, vfo_t vfo, tone_t tone)
{
   /* same opcode as tone */
   return ft847old_set_ctcss_tone(rig, vfo, tone);
}

int ft847old_set_dcs_sql (RIG *rig, vfo_t vfo, tone_t code)
{
  unsigned char p_cmd[YAESU_CMD_LENGTH]; /* sequence to send */
  int ret;
...
int ft847old_set_rptr_shift (RIG *rig, vfo_t vfo, rptr_shift_t rptr_shift)
{
  unsigned char cmd_index;	/* index of sequence to send */
...
int ft847old_set_rptr_offs (RIG *rig, vfo_t vfo, shortfreq_t rptr_offs)
{
  unsigned char p_cmd[YAESU_CMD_LENGTH]; /* sequence to send */
