...
#ifndef _FT847OLD_H
#define _FT847OLD_H 1


#define FT847OLD_WRITE_DELAY                    50

/* Sequential fast writes may confuse FT847 without this delay */

#define FT847OLD_POST_WRITE_DELAY               50


/* Rough safe value for default timeout */

#define FT847OLD_DEFAULT_READ_TIMEOUT           2000
...
typedef enum ft847old_native_cmd_e ft847old_native_cmd_t;
...
static int ft847old_init(RIG *rig);
static int ft847old_open(RIG *rig);

static int ft847old_cleanup(RIG *rig);
static int ft847old_close(RIG *rig);

static int ft847old_set_freq(RIG *rig, vfo_t vfo, freq_t freq);

static int ft847old_set_mode(RIG *rig, vfo_t vfo, rmode_t mode, pbwidth_t width); /* select mode */

static int ft847old_set_split_vfo(RIG *rig, vfo_t vfo, split_t split, vfo_t tx_vfo);
static int ft847old_get_split_vfo(RIG *rig, vfo_t vfo, split_t *split, vfo_t *tx_vfo);

static int ft847old_set_split_freq(RIG *rig, vfo_t vfo, freq_t freq);

static int ft847old_set_split_mode(RIG *rig, vfo_t vfo, rmode_t mode, pbwidth_t width);

static int ft847old_set_ptt(RIG *rig, vfo_t vfo, ptt_t ptt);
static int ft847old_get_ptt(RIG *rig, vfo_t vfo, ptt_t *ptt);
static int ft847old_get_dcd(RIG *rig, vfo_t vfo, dcd_t *dcd);

static int ft847old_get_level(RIG *rig, vfo_t vfo, setting_t level, value_t * val);

static int ft847old_set_func(RIG *rig, vfo_t vfo, setting_t func, int status);
static int ft847old_set_ctcss_tone (RIG *rig, vfo_t vfo, tone_t tone);
static int ft847old_set_ctcss_sql (RIG *rig, vfo_t vfo, tone_t tone);
static int ft847old_set_dcs_sql (RIG *rig, vfo_t vfo, tone_t code);
static int ft847old_set_rptr_shift (RIG *rig, vfo_t vfo, rptr_shift_t rptr_shift);
static int ft847old_set_rptr_offs (RIG *rig, vfo_t vfo, shortfreq_t rptr_offs);
